...
const getFewArticles=(start,num,sort,title)=>{
    ...
    if(sort){
        if(title){
            _sql=`SELECT t.* FROM( select a.* ,COUNT(b.topic_id) AS reply_count FROM (SELECT * FROM articles where sort=${sort} and title like '%${title}%' order by id desc limit ${start},${num}) a 
            LEFT JOIN article_comment b ON a.id = b.topic_id GROUP BY a.id) t
            ORDER BY -t.reply_count`;
        }else{
            _sql=`SELECT t.* FROM( select a.* ,COUNT(b.topic_id) AS reply_count FROM (SELECT * FROM articles where sort=${sort} order by id desc limit ${start},${num}) a 
            LEFT JOIN article_comment b ON a.id = b.topic_id GROUP BY a.id) t
            ORDER BY -t.reply_count`;
        }
    }else{
        if(title){
            _sql=`SELECT t.* FROM( select a.* ,COUNT(b.topic_id) AS reply_count FROM (SELECT * FROM articles where title like '%${title}%' order by id desc limit ${start},${num}) a 
            LEFT JOIN article_comment b ON a.id = b.topic_id GROUP BY a.id) t
        ORDER BY -t.reply_count`;
        }else{
            _sql=`SELECT t.* FROM( select a.* ,COUNT(b.topic_id) AS reply_count FROM (SELECT * FROM articles limit ${start},${num}) a 
                LEFT JOIN article_comment b ON a.id = b.topic_id GROUP BY a.id) t
            ORDER BY -t.reply_count`;
        }
    }
