...
const getFewArticles=(start,num,sort,title)=>{
    if(sort){
        if(title){
            _sql=`select * from articles where sort=${sort} and title like '%${title}%' limit ${start},${num}`;
        }else{
            _sql=`select * from articles where sort=${sort} limit ${start},${num}`;
        }
    }else{
        if(title){
            _sql=`select * from articles where title like '%${title}%' limit ${start},${num}`;
        }else{
            _sql=`select * from articles limit ${start},${num}`;
        }
    }
    return query(_sql);
};
