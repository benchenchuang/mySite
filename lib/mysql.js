...
const findArticleById=(id)=>{
    _sql=`select * from articles where id='${id}'`;
    return query(_sql)
};
//创建文章
...
const getFewArticles=(start,num,sort,title)=>{
    if(sort){
        if(title){
            _sql=`select * from articles where sort=${sort} and title like '%${title}%' order by id desc limit ${start},${num}`;
        }else{
            _sql=`select * from articles where sort=${sort} order by id desc limit ${start},${num}`;
        }
    }else{
        if(title){
            _sql=`select * from articles where title like '%${title}%' order by id desc limit ${start},${num}`;
        }else{
            _sql=`select * from articles order by id desc limit ${start},${num}`;
        }
    }
    return query(_sql);
};
